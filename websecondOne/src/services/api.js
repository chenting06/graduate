...
export async function getAuthorDishes() {
  let result;
  axios.defaults.withCredentials = true;
  await axios
    .get("http://localhost:7777/api/dishes/getAuthorDishes")
    .then((res) => {
      console.log(res);
      result = res;
    })
...
// 分页获取所有菜品
export async function getAllDishes(page, size) {
  axios.defaults.withCredentials = true;
  let result;
  await axios
...
export async function guessYouLike() {
  ...
  let result;
  await axios
    .post("http://localhost:7777/api/dishes/guessYouLike")
    .then((res) => {
      if (res.data.error_code === 1) {
        alert(res.data.msg);
      }
...
export async function searchDishes(searchContent) {
  let result;
  await axios
    .post("http://localhost:7777/api/users/searchDishes", { searchContent })
    .then((res) => {
      console.log(res);
      result = res;
    })
