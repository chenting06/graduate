...
export default function EditDish() {
  ...
  useEffect(() => {
    ...
    let materNumTemple = [];
    let stepNumTemple = [];
    console.log(dish.dishTags.length);
    ...
    // }
    for (let i = 0; i < dish.dishTags.length; i++) {
      materNumTemple.push(i);
    }
    for (let i = 0; i < dish.dishStep.length; i++) {
  ...
  async function changePicResult(item) {
    ...
    if (picTemple.length === stepNum.length) {
      ...
      for (let i = 0; i < stepNum.length; i++) {
        ...
        let objTemple = {
          pic: stepPicTemple[indexTemple].picId
            ? stepPicTemple[indexTemple].picId
            : dish.dishStep[i]
            ? dish.dishStep[i].pic
            : "",
                ...
                <div className="editStep">
                  ...
                  <div className="content">
                    <textarea
                      ...
                      rows="5"
                      placeholder={
                        dish.dishStep[item] ? dish.dishStep[item].content : ""
                      }
                    ></textarea>
                  </div>
                </div>
