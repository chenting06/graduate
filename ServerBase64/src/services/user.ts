...
export default class UserService {
  ...
  // }
  public async searchDishes(searchContent: string, startPage: number) {
    ...
    let dishResultTemple;
    if (startPage * 6 < dishResult.length) {
      dishResultTemple = dishResult.splice(startPage * 6, 6);
      dishResultTemple.map(function (item: any) {
        item.dishStep = item.dishStep.splice(0, 1);
      });
    } else {
      dishResultTemple = null;
    }

    return dishResultTemple;
  }
  public async getUserLike(userId: string, startPage: number) {
    // console.log(userId);

    let user = await User.findById(userId);
    let dishIdArr = user.userLike;

    let dishArr = [];
    // dishIdArr.map(function(item:any){
    for (let i = 0; i < dishIdArr.length; i++) {
      let dishTemple = await dishes.findById(dishIdArr[i]);
      if (dishTemple) {
        dishTemple.dishStep = dishTemple.dishStep.splice(0, 1);
        dishArr.push(dishTemple);
      }
    }

    let dishArrTemple = dishArr.splice(startPage * 6, 6);
    console.log(dishArrTemple.length);
    return dishArrTemple;
    // })
  }
  public async getUserDislike(userId: string, startPage: number) {
    let user = await User.findById(userId);
    let dishIdArr = user.userDislike;
    let dishArr = [];
    // dishIdArr.map(function(item:any){
    for (let i = 0; i < dishIdArr.length; i++) {
      let dishTemple = await dishes.findById(dishIdArr[i]);
