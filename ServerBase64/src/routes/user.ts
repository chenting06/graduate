...
userRouter
  ...
  .post("/searchDishes", async (ctx: Context) => {
    ...
    let startPage = payload.startPage;
    let data: any = await userService.searchDishes(searchContent, startPage);
    if (data) {
      createRes({
        ctx,
        data,
  ...
  .post("/getUserLike", async (ctx: Context) => {
    try {
      const payload = ctx.request.body;
      const { startPage } = payload;
      let userId = ctx.cookies.get("userid");
      let data = await userService.getUserLike(userId, startPage);

      if (data) {
        createRes({ data, ctx, statusCode: 201 });
      } else {
        createRes({ data, ctx, statusCode: 200 });
      }
    } catch (error) {
      createRes({ ctx, msg: error, errorCode: 1 });
    }
  })
  .post("/getUserDislike", async (ctx: Context) => {
    try {
      const payload = ctx.request.body;
      const { startPage } = payload;
      let userId = ctx.cookies.get("userid");
      let data = await userService.getUserDislike(userId, startPage);
